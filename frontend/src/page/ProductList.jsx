...
const ProductList = () => {
  ...
  const searchProducts = async (keyword) => {
    try {
      ...
      // Thử gọi search với các tham số khác nhau để lấy tất cả kết quả
      try {
        const response = await axiosInstance.post('/search', { keyword, limit: 1000 })
        if (response.data.status === 'success') {
          const data = response.data.data
          const productsList = Array.isArray(data) ? data : (data?.products || [])
          const pagination = data?.pagination
          
          if (pagination && pagination.total > productsList.length) {
            // Nếu có pagination và còn nhiều kết quả, lấy các trang còn lại
            allSearchResults = [...productsList]
            const totalPages = Math.ceil(pagination.total / (pagination.per_page || 20))
            
            for (let page = 2; page <= totalPages; page++) {
              try {
                const pageResponse = await axiosInstance.post('/search', { 
                  keyword, 
                  page 
                })
                if (pageResponse.data.status === 'success') {
                  const pageData = pageResponse.data.data
                  const pageProducts = Array.isArray(pageData) ? pageData : (pageData?.products || [])
                  allSearchResults = [...allSearchResults, ...pageProducts]
                }
              } catch (pageError) {
                if (process.env.NODE_ENV === 'development') {
                  console.warn(`ProductList Search - Lỗi khi lấy trang ${page}:`, pageError)
                }
              }
            }
          } else {
        ...
        // Fallback: Thử không có tham số limit
        try {
          const fallbackResponse = await axiosInstance.post('/search', { keyword })
          if (fallbackResponse.data.status === 'success') {
            const data = fallbackResponse.data.data
            allSearchResults = Array.isArray(data) ? data : (data?.products || [])
          }
        } catch (fallbackErr) {
          throw fallbackErr
        }
      ...
      const activeProducts = allSearchResults.filter(product => product.status_product === 0)
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`ProductList Search - Đã tìm thấy ${activeProducts.length} sản phẩm`)
      }
    } catch (error) {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Thử các cách khác nhau để lấy tất cả sản phẩm
      const strategies = [
        // Strategy 1: Thử với limit lớn
        { params: { limit: 1000 } },
        // Strategy 2: Thử với per_page lớn
        { params: { per_page: 1000 } },
        // Strategy 3: Thử với all=true
        { params: { all: true } },
        // Strategy 4: Thử với page=all
        { params: { page: 'all' } },
        // Strategy 5: Gọi nhiều trang để lấy tất cả
        null // Sẽ xử lý riêng
      ]
      
      let success = false
      
      // Thử các strategy từ 1-4
      for (let i = 0; i < strategies.length - 1; i++) {
        try {
          const response = await axiosInstance.get('/products', strategies[i])
          if (response.data.status === 'success') {
            const data = response.data.data
            const productsList = Array.isArray(data) ? data : (data?.products || [])
            ...
            if (process.env.NODE_ENV === 'development') {
              console.log(`ProductList - Strategy ${i + 1} - Số lượng sản phẩm:`, productsList.length)
              if (pagination) {
                console.log('ProductList - Pagination info:', pagination)
              }
            }
            
            // Nếu lấy được nhiều hơn 20 sản phẩm hoặc không có pagination, coi như thành công
            if (productsList.length > 20 || !pagination || productsList.length === pagination.total) {
              allProductsFromAPI = productsList
              success = true
              break
            }
          }
      ...
      // Strategy 5: Nếu chưa thành công, gọi nhiều trang để lấy tất cả
      if (!success) {
        try {
          const firstPageResponse = await axiosInstance.get('/products')
          if (firstPageResponse.data.status === 'success') {
            const data = firstPageResponse.data.data
            const pagination = data?.pagination
            const firstPageProducts = Array.isArray(data) ? data : (data?.products || [])
            
            if (pagination && pagination.total > firstPageProducts.length) {
              if (process.env.NODE_ENV === 'development') {
                console.log(`ProductList - Tổng số sản phẩm: ${pagination.total}, đang lấy từng trang...`)
              }
              allProductsFromAPI = [...firstPageProducts]
              ...
              for (let page = 2; page <= totalPages; page++) {
                try {
                  const pageResponse = await axiosInstance.get('/products', {
                    params: { page }
                  })
                  if (pageResponse.data.status === 'success') {
                    const pageData = pageResponse.data.data
                    const pageProducts = Array.isArray(pageData) ? pageData : (pageData?.products || [])
                    allProductsFromAPI = [...allProductsFromAPI, ...pageProducts]
                  }
                } catch (pageError) {
                  if (process.env.NODE_ENV === 'development') {
                    console.warn(`ProductList - Lỗi khi lấy trang ${page}:`, pageError)
                  }
                }
              }
              
              if (process.env.NODE_ENV === 'development') {
                console.log(`ProductList - Đã lấy tổng cộng ${allProductsFromAPI.length} sản phẩm từ ${totalPages} trang`)
              }
              success = true
            } else {
              allProductsFromAPI = firstPageProducts
          ...
          if (process.env.NODE_ENV === 'development') {
            console.error('ProductList - Lỗi khi lấy sản phẩm theo trang:', err)
          }
        }
      }
      
      if (!success || allProductsFromAPI.length === 0) {
        // Fallback: Lấy trang đầu tiên
        const fallbackResponse = await axiosInstance.get('/products')
        if (fallbackResponse.data.status === 'success') {
          const data = fallbackResponse.data.data
          allProductsFromAPI = Array.isArray(data) ? data : (data?.products || [])
          if (process.env.NODE_ENV === 'development') {
            console.warn('ProductList - Chỉ lấy được trang đầu tiên:', allProductsFromAPI.length, 'sản phẩm')
          }
        }
      }
      
      // Chỉ lấy sản phẩm đang hoạt động (status_product === 0)
      const activeProducts = allProductsFromAPI.filter(product => product.status_product === 0)
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`ProductList - Đã tải thành công ${activeProducts.length} sản phẩm đang hoạt động`)
      }
    } catch (error) {
